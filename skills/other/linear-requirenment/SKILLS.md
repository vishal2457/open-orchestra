...
- Read `workflow.agents.requirements-ticket-agent` to determine:
...
- If `dependsOn` lists agents that are not complete yet, stop and report that requirements ticketing is blocked by workflow configuration.
